...
class Dataset(object):
    ...
    def _parse_dataset(self, dataset_filepath):
        ...
        if dataset_filepath:
            ...
            if len(new_token_sequence) > 0:
                ...
                patterns.append(new_pattern_sequence)
            f.close()
        return labels, tokens, token_count, label_count, character_count , patterns
    ...
    def update_dataset(self, dataset_filepaths, dataset_types):
        ...
        '''
        for dataset_type in dataset_types:
            self.labels[dataset_type], self.tokens[dataset_type], _, _, _ ,self.token_patterns[dataset_type] = self._parse_dataset(
    ...
    #         character_count[dataset_type] ,self.token_patterns[dataset_type] \
    ...
    #
    #     self.size_pattern_vector = len(self.token_patterns['train'][0][0])
    ...
    def load_dataset(self, dataset_filepaths,token_pretrained_embedding_filepath, parameters,tagging_format='bioes',load_only_pretrained_token_embeddings=False,load_all_pretrained_token_embeddings=False, token_to_vector=None, pretraining_dataset = None, remap_unknown_tokens_to_unk = True):
        '''
        dataset_filepaths : dictionary with keys 'train', 'valid', 'test', 'deploy'
        '''
        start_time = time.time()
        ...
        if token_pretrained_embedding_filepath != '':
            if token_to_vector == None:
                token_to_vector = utils_nlp.load_pretrained_token_embeddings(token_pretrained_embedding_filepath)
        else:
            token_to_vector = {}
        if self.verbose: print("len(token_to_vector): {0}".format(len(token_to_vector)))
        ...
        if load_all_pretrained_token_embeddings:
            for token in token_to_vector:
                if token not in token_count['all']:
                    token_count['all'][token] = -1
                    token_count['train'][token] = -1
        ...
        number_of_unknown_tokens = 0
        if self.verbose:
            print("parameters['remap_unknown_tokens_to_unk']: {0}".format(remap_unknown_tokens_to_unk))
        if self.verbose:
            print("len(token_count['train'].keys()): {0}".format(len(token_count['train'].keys())))
        for token, count in token_count['all'].items():
            if iteration_number == self.UNK_TOKEN_INDEX: iteration_number += 1

            if remap_unknown_tokens_to_unk == 1 and \
                    (token_count['train'][token] == 0 or \
                             load_only_pretrained_token_embeddings) and \
                    not utils_nlp.is_token_in_pretrained_embeddings(token, token_to_vector, parameters) and \
                            token not in all_tokens_in_pretraining_dataset:
                if self.verbose: print("token: {0}".format(token))
                if self.verbose: print("token.lower(): {0}".format(token.lower()))
        ...
        index_to_token = utils.reverse_dictionary(token_to_index)
        if remap_unknown_tokens_to_unk == 1:
