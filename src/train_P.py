...
def predict_labels(sess, model, transition_params_trained, dataset, epoch_number, stats_graph_folder, dataset_filepaths,tagging_format):
    ...
    for dataset_type in ['train', 'valid', 'test', 'deploy']:
        if dataset_type not in dataset_filepaths.keys():
            continue
        prediction_output = prediction_step(sess, dataset, dataset_type, model, transition_params_trained, stats_graph_folder, epoch_number, dataset_filepaths,tagging_format=tagging_format )
...
def prediction(sess, dataset, dataset_type, model, transition_params_trained,tagging_format='bioes'):
    # if dataset_type == 'deploy':
    #     print('Predict labels for the {0} set'.format(dataset_type))
    # else:
    #     print('Evaluate model on the {0} set'.format(dataset_type))
...
def evaluate_step(sess, dataset, dataset_type, model, transition_params_trained,tagging_format):
    if dataset_type == 'deploy':
        print('Predict labels for the {0} set'.format(dataset_type))
    else:
        print('Evaluate model on the {0} set'.format(dataset_type))
